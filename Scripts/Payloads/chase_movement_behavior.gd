...
func get_position_delta(payload_owner, direction, payloadStatsResource: PayloadStats):
	...
	if enemy_target:
		...
		payload_owner.rotation = lerp_angle(payload_owner.rotation, payload_owner.target_rotation, payloadStatsResource.rotation_speed * get_process_delta_time())
		var move_direction = Vector2(cos(payload_owner.rotation), sin(payload_owner.rotation))
		return move_direction * payloadStatsResource.bullet_speed * get_process_delta_time()
	else:
